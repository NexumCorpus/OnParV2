...
// Auto-generate draft POs from all low-stock items grouped by supplier
export async function autoGenerateDraftPOs(): Promise<ActionResult> {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()
    ...
    if (error) {
      logger.error({ err: error, action: 'autoGenerateDraftPOs' }, 'Failed to fetch items')
      return { success: false, error: 'Failed to fetch inventory items' }
    }

    // Filter to low-stock items that have a supplier
    const lowStockItems = (items ?? []).filter(
      (i: { quantity: number; reorder_point: number; supplier_id: string | null }) =>
        i.quantity <= i.reorder_point && i.supplier_id
    )

    if (lowStockItems.length === 0) {
      return { success: false, error: 'No low-stock items with assigned suppliers found' }
    }

    // Group by supplier
    const bySupplier = new Map<string, typeof lowStockItems>()
    for (const item of lowStockItems) {
      const sid = item.supplier_id as string
    ...
    for (const [supplierId, supplierItems] of bySupplier) {
      const poItems = supplierItems.map((item: { id: string; max_stock_level: number | null; quantity: number; reorder_point: number; price_per_unit: number }) => ({
        id: item.id,
        quantity: Math.max(
          item.reorder_point,
          (item.max_stock_level ?? item.reorder_point * 2) - item.quantity
        ),
        price: item.price_per_unit,
      }))

      const result = await createDraftPO(supplierId, poItems)
      if (result.success) {
        createdPOs.push(supplierId)
      }
    }
    ...
    return {
      success: true,
      data: { count: createdPOs.length, supplierCount: bySupplier.size },
    }
  } catch (err) {
    logger.error({ err, action: 'autoGenerateDraftPOs' }, 'Error auto-generating POs')
    return { success: false, error: 'Something went wrong.' }
  }
}
