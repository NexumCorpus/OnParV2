...
import { updateRecipe, addIngredient, removeIngredient } from '@/lib/actions/recipes'
import { formatCurrency, formatPercentage } from '@/lib/utils/formatting'
import { IngredientPicker, type IngredientRow } from './ingredient-picker'
import { toast } from 'sonner'
import type { Recipe, InventoryItem, RecipeIngredient } from '@/types'
...
export function EditRecipeDialog({
  ...
  const fetchRecipeDetails = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const rows: IngredientRow[] = data.ingredients.map((ing) => ({
          id: ing.id,
          inventory_item_id: ing.inventory_item_id,
          inventory_item_name: ing.inventory_item?.name ?? 'Unknown',
          quantity_needed: ing.quantity_needed,
          unit: ing.unit,
          cost_per_unit: ing.cost_per_unit,
        }))
        setIngredients(rows)
        setOriginalIngredientIds(rows.map((r) => r.id).filter((id): id is string => !!id))
      }
    } catch {
  ...
  }, [open, recipe, reset, fetchRecipeDetails])
  ...
  const onSubmit = async (data: RecipeFormValues) => {
    ...
    // Sync ingredients: remove deleted, add new
    const currentIds = ingredients
      .map((ing) => ing.id)
      .filter((id): id is string => !!id)
    const removedIds = originalIngredientIds.filter((id) => !currentIds.includes(id))
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          {/* Description */}
          <div className="space-y-1">
            <Label htmlFor="edit-recipe-desc">Description</Label>
            <textarea
              id="edit-recipe-desc"
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="space-y-1">
              <Label htmlFor="edit-serving_size">Servings *</Label>
              <Input
            ...
            <div className="space-y-1">
              <Label htmlFor="edit-prep_time">Prep (min)</Label>
              <Input
                id="edit-prep_time"
                type="number"
                min={0}
            ...
            <div className="space-y-1">
              <Label htmlFor="edit-cook_time">Cook (min)</Label>
              <Input
                id="edit-cook_time"
                type="number"
                min={0}
            ...
            <div className="space-y-1">
              <Label htmlFor="edit-selling_price">Sell Price *</Label>
              <Input
