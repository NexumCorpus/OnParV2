...
import { AlertTriangle, Clock, AlertCircle, ShoppingCart } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { toast } from 'sonner'
import { autoGenerateDraftPOs } from '@/lib/actions/purchasing'
import type { InventoryItem } from '@/types'

interface RecentAlertsProps {
  lowStockItems: InventoryItem[]
  expiringItems: InventoryItem[]
  hasSupplierLinks?: boolean
}

export function RecentAlerts({ lowStockItems, expiringItems, hasSupplierLinks }: RecentAlertsProps) {
  const [generatingPOs, setGeneratingPOs] = useState(false)

  const handleAutoGeneratePOs = async () => {
    setGeneratingPOs(true)
    try {
      const result = await autoGenerateDraftPOs()
      if (result.success) {
        const data = result.data as { count: number }
        toast.success(`Created ${data.count} draft purchase order${data.count !== 1 ? 's' : ''}`)
      } else {
  ...
  return (
    <div className="rounded-lg border bg-card p-4">
      ...
      {hasSupplierLinks && lowStockItems.length > 0 && (
        <div className="mt-3 pt-3 border-t">
          ...
          >
            ...
            {generatingPOs ? 'Generating...' : 'Auto-Generate Purchase Orders'}
          </Button>
        </div>
      )}
    </div>
